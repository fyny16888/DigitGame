...
var User = require('../fruit/user');
var C = require('../../share/constant');
var H = require('../../share/const').TASK_TYPE;
var logger = quick.logger.getLogger('fruit', __filename);
...
var Controller = function (app) {
    ...
    this.gameBets = {
        ...
        banker: '0',
        bets: [
            { _id: 1, count: 0, bet: 0 },			// 水果1
            { _id: 2, count: 0, bet: 0 },			// 水果2
            { _id: 3, count: 0, bet: 0 },			// 水果3
            { _id: 4, count: 0, bet: 0 },			// 水果4
            { _id: 5, count: 0, bet: 0 },			// 水果5
            { _id: 6, count: 0, bet: 0 },			// 水果6
            { _id: 7, count: 0, bet: 0 },			// 水果7
            { _id: 8, count: 0, bet: 0 },			// 水果8
            { _id: 9, count: 0, bet: 0 },			// 水果9
        ]
    };
    // 同步DB定时器
...
proto.notifyAllAsync = P.coroutine(function* (route, msg) {
    ...
    if (this.app.getServerId() == mainServer.id) {
        ...
        return fruitRemote.pushAll.toServer(mainServer.id, route, msg, () => { });
    }
});
...
proto.beforeJoinGameAsync = P.coroutine(function* (user) {
    ...
    if (this.app.getServerId() != mainServer.id) {
        let fruitRemote = this.app.rpc.fruit.fruitRemote;
        return fruitRemote.joinChannel.toServer(mainServer.id, channelId, user.id, connectorId, () => { });
    }
});
...
proto.beforeLeaveGameAsync = P.coroutine(function* (user) {
    ...
    if (this.app.getServerId() != mainServer.id) {
        let fruitRemote = this.app.rpc.fruit.fruitRemote;
        return fruitRemote.quitChannel.toServer(mainServer.id, channelId, user.id, () => { });
    }
});
...
proto.startGameAsync = P.coroutine(function* (startTime) {
    ...
    // 通知开始下注
    return this.notifyAsync('fruit_startBet', { time: String(Math.round((openTime - nowTime) / 1000)) });
});
...
proto.stopBetAsync = P.coroutine(function* () {
    ...
    // 通知停止下注
    return this.notifyAsync('fruit_stopBet');
});
...
proto.endGameAsync = P.coroutine(function* () {
    ...
    if (this.app.getServerId() == mainServer.id) {
        ...
        let fruitRemote = this.app.rpc.fruit.fruitRemote;
        fruitRemote.collectResult.toServer(mainServer.id, maxId, maxVal, gameBets.gift, () => { });
    }
    // 清空限制
    if (this.app.getServerId() == mainServer.id) {
        yield P.promisify((cb) => {
            var betsRemote = this.app.rpc.checker.betsRemote;
            return betsRemote.resetFruit.toServer('bets-check-server', cb);
        })();
    }
    // 开奖结果
    return this.notifyAsync('fruit_result', data);
});
...
proto.changeBankerAsync = P.coroutine(function* (banker, bankCount, pos) {
    ...
    return this.notifyAsync('fruit_changeBanker', data);
});
...
proto.betGoldAsync = P.coroutine(function* (playerId, bets) {
    ...
    if (vbets.length > 0) {
        let res = yield P.promisify((betTol, cb) => {
            var betsRemote = this.app.rpc.checker.betsRemote;
            return betsRemote.checkFruitCanBet.toServer('bets-check-server', betTol, cb);
        })(ttBet);
        if (!res.can) {
...
proto.changeBanksNotFull = function (orRpc) {
    ...
    if (orRpc) {
        ...
        let gameServers = this.app.getServersByType('fruit');
		let fruitRemote = this.app.rpc.fruit.fruitRemote;
        for (let i = 0; i < gameServers.length; ++i) {
            if (gameServers[i].id == serverId) continue;
            fruitRemote.changeBanksNotFull.toServer(gameServers[i].id, false, () => {});
        }
    }
};
...
proto.collectResultAsync = P.coroutine(function* (maxId, maxVal, gift) {
    ...
    if (this.svrCount >= gameServers.length) {
        if (this.maxVal > this.gmaxVal) {
            this.gmaxId = this.maxId;
            this.gmaxVal = this.maxVal;
        }
        let data = {
            caijin: String(this.gift),
            max: { name: '', sex: '0', headurl: '', vip: 0, value: String(this.maxVal) }
        }
        if (this.maxId) {
            let player = yield this.app.models.Player.findByIdReadOnlyAsync(this.maxId, 'name sex headurl vip');
            if (player) {
        ...
        return this.notifyAllAsync('fruit_collect', data)
            .then(() => {
                this.gift = 0;
                this.maxId = '';
                this.maxVal = 0;
...
proto.broadcastAsync = P.coroutine(function* (playerId, msg) {
    ...
    if (!playerId) {
        return this.notifyAsync('fruit_gameChat', msg).then(res);
    }
    if (playerId != '0') {
    ...
    return this.notifyAllAsync('fruit_gameChat', msg).then(res);
});
